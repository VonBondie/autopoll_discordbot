...
class AddCandidateModal(ui.Modal, title='候補の登録'):
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    if re.fullmatch(pattern, self.weight.value) == None:
      ...
      return
    updateCandidates(str(interaction.guild_id), self.name.value, self.weight_transform_dict[self.weight.value], self.description.value)
    mes = self.name.value + "が候補に登録されました。"
    await interaction.response.send_message(mes, ephemeral=False)
...
def createPollEmb(title:str, guild_id: str):
  desc = ""
  guild_candidates = candidates[guild_id]
  for i, game in enumerate(guild_candidates):
    info = ""
    info += emoji_list[i] + " `" + str(i) + " : "
    info += guild_candidates[game]["weight"] + " "
    info += game + " : "
    info += guild_candidates[game]["description"] + '`\n'
    desc += info
...
async def create_poll(interaction: discord.Interaction, poll_title: str):
  ...
  if len(guild_candidates) == 0:
    await interaction.response.send_message("候補が登録されていません。", ephemeral=True)
    return
  await interaction.response.send_message(embed=createPollEmb(poll_title, str(interaction.guild_id)))
  async for message in interaction.channel.history(limit=1):
    for i in range(len(guild_candidates)):
...
async def show_candidates(interaction: discord.Interaction):
  ...
  if len(guild_candidates) == 0:
    await interaction.response.send_message("候補が登録されていません。", ephemeral=True)
    return
  await interaction.response.send_message(embed=createPollEmb("次回のゲーム候補一覧", str(interaction.guild_id)), ephemeral=True)
  
@tree.command(name="remove_candidate", description="候補の削除")
async def remove_candidate(interaction: discord.Interaction, id:int):
  guild_candidates = candidates[str(interaction.guild_id)]
  if id < 0 or id >= len(guild_candidates):
    await interaction.response.send_message("idが不適です。", ephemeral=True)
    return
  for i, key in enumerate(guild_candidates):
    if i != id:
      continue
    guild_candidates.pop(key)
